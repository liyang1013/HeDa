...
var vm = new Vue({
	...
	methods: {
		...
		openTabs: function (e) { //打开页面 加载显示tabs
			...
			if (this.tabs.length == 0) {
				this.tabs.push({ id: e.menuid, data: e })
				this.id = e.menuid
				this.item = e
				calWidth()
			} else {
				var back = checkTab(e.menuid)
				if (back == 1) { //可以插入
					this.tabs.push({ id: e.menuid, data: e })
					this.id = e.menuid
					this.item = e
		...
		personCommand(e){
			 switch(e){
				 case 'changePassword': this.$message("修改密码");break;
				 case 'logout':
				 this.$confirm('确定退出登录吗?', '提示', {
				         ...
				         axios.get("/user/logout").then(response => location.href = "/").catch(error => console.log(error));
				         }).catch(() => {
	...
	created(){
		axios.get("/user/userInfo")
			.then(response => {
				if(response.data.result === null) location.href = "/";
				else {
					this.user = response.data.result;
					axios.post("/menu/getMenuList",this.user)
						.then(response=>this.navs = response.data.result)
...
function closeTabOther() { //删除tabs所有数组，重新push
	if (vm.tabs.length == 0) {
		...
		if (vm.id == 0) {
			...
			var item = vm.item
			vm.tabs = []
			vm.tabs.push({ id: item.menuid, data: item })
			$('.tab-item-ul').css("left", 0)
		}
	}
}
